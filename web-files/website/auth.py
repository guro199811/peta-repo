# This py file is intended for authentification logic
# as well as everything thats involved with it
...
from . import db
from .models import (
    Person, PhonePrefixes, PersonToClinic, Clinic,
    Visit
)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.temporary_block and datetime.utcnow() < \
                    user.temporary_block:
                flash(
                    _('თქვენი აქაუნთი დროებით დაბლოკილია,' +
                      ' გთხოვთ სცადოთ მოგვიანებით.'), 'error')
                return render_template(
                    'login.html', user=current_user, tries=session['tries']
                    )
            ...
            if check_password_hash(user.password, password):
                ...
                if user.confirmed:
                    ...
                    match user_type:
                        ...
                        case _:
                            return render_template(
                                "auths/verification.html",
                                user=current_user, verification_type=0)
            else:
                ...
                if user.login_attempts >= 10:
                    ...
                    user.temporary_block = datetime.utcnow() + \
                        timedelta(minutes=30)
                    db.session.commit()
                    flash(_('10 წარუმატებელი ცდის შედეგად,' +
                            ' აქაუნთი დაბლოკილია 30 წუთით.'), category='error')
                else:
                    error_message = _('პაროლი არასწორია,' +
                                      ' გთხოვთ სცადოთ ხელახლა.' +
                                      ' ცდების რაოდენობა დარჩენილია = {tries}')
                    flash(error_message.format(
                        tries=10 - session["tries"]), 'error')

                return render_template(
                    'login.html', user=current_user, tries=session['tries']
                    )
        else:
            flash(_('ელ.ფოსტა არ არის დარეგისტრირებული,' +
                    ' გთხოვთ სცადოთ ხელახლა.'), 'error')
    return render_template(
        "login.html", user=current_user, tries=session['tries']
        )
...
def register():
    ...
    if request.method == "POST":
        ...
        if user:
            flash(_('მოცემული ელ.ფოსტა უკვე დარეგისტრირებულია.'),
                  category='error')
        elif len(mail) < 4:
            flash(_("ემაილი უნდა შეიცავდეს მინიმუმ 4 სიმბოლოს."),
                  category="error")
        elif len(name) < 2:
            flash(_("სახელი უნდა იყოს მინიმუმ 1 ასოზე დიდი."),
                  category="error")
        elif password1 != password2:
            flash(_("გამეორებული პაროლი არაა სწორი"),
                  category="error")
        elif len(password1) < 6:
            flash(_("პაროლის მინიმალური ზომა არის 6 სიმბოლო."),
                  category="error")
        elif not re.search("[a-zA-Z]", password1):
            flash(_("თქვენი მაროლი უნდა შეიცავდეს მინიმუმ 1 ასოს"),
                  category="error")
    ...
    return render_template("sign-up.html",
                           user=current_user, prefixes=all_prefixes)
...
def send_confirmation(new_user=current_user):
    ...
    message = Message(_('ელ.ფოსტის დასტური(Petaworld.com)'),
                      sender='noreply@peta.ge', recipients=[new_user.mail])
...
def confirm_token(token, expiration=3600):
    ...
    try:
        ...
        return render_template('auths/expired-token.html',
                               user=current_user, expiredType=0)
...
def forgot_password():
    if request.method == 'POST':
        ...
        if user:
            ...
            success_message = _('პაროლის შეცვლის იმეილი ' +
...
def send_password_reset_email(user):
    ...
    message = Message(_('პაროლის შეცვლა (Petaworld.com)'),
                      sender='noreply@peta.ge', recipients=[user.mail])
...
def reset_password(token):
    if request.method == 'POST':
        ...
        if new_password != confirm_password:
            ...
            flash(_('პაროლი უნდა შეიცავდეს მინიმუმ 6 სიმბოლოს'),
                  category='error')
    ...
    return render_template("auths/reset_password.html",
...
def confirm_password_token(token, expiration=3600):
    ...
    try:
        ...
        return render_template(
            'auths/expired-token.html', user=current_user, expiredType=1
            )
...
def clinic_removal_email(clinic_id):
    ...
    remove_url = url_for(
        'auth.confirm_clinic_removal', clinic_id=clinic_id,
        token=token, _external=True)
    r_mail = Mail(app)

    # checking if user is owner of that clinic for different messege
    try:
        person = db.session.query(PersonToClinic).\
            filter_by(clinic_id=clinic_id,
                      person_id=current_user.id).one_or_none()
        clinic = db.session.query(Clinic).\
            filter_by(clinic_id=clinic_id).one()
        if person.is_clinic_owner:
            message = Message(_('დასტური კლინიკის წაშლაზე (Petaworld.com)'),
                              sender='noreply@peta.ge', recipients=[user.mail])
            m = 'მოგესალმებით, გთხოვთ წაიკითხოთ!\n\nთქვენ ხართ მოცემული \
                კლინიკის "{clinic.clinic_name}" დამრეგისტრირებელი,\n\n\
                მოცემული კლინიკის წაშლით,ანუ ბმულზე გადასვლის შედეგად \
                თქვენ დარწმუნებული ხართ რომ შლით:\n\n\
                • ყველა კავშირს მოცემულ კლინიკასთან (აღდგენა შეუძლებელია)\n\
                • ყველა ვიზიტს რომელიც მოცემულ კლინიკაშია \
                დარეგისტრირებული (აღდგენა შეუძლებელია)\n\
                • ყველა თანამშრომლების კავშირებს კლინიკასთან \
                (აღდგენა შეუძლებელია)\n\n\n\
                {remove_url}\nლინკი გაუქმდება გამოგზავნიდან \
                1 საათში.\n\n\nPeta-Team'
            message.body = m.format(clinic.clinic_name, remove_url)
        else:
            message = Message(_('დასტური კლინიკასთან კავშირის გაწყვეტაზე \
                                (Petaworld.com)'), sender='noreply@peta.ge',
                              recipients=[user.mail])
            m = 'თუ დარწმუნებული ხართ რომ გნებავთ კავშირის გაწყვეტა \
                კლინიკასთან სახელად "{clinic.clinic_name}", გადადით \
                    მოცემულ ბმულზე: {remove_url}\nლინკი გაუქმდება \
                        გამოგზავნიდან 1 საათში.\n\n\nPeta-Team'
            message.body = m.format(clinic.clinic_name, remove_url)
...
def confirm_clinic_removal(clinic_id, token, expiration=3600):
    ...
    try:
        email = serializer.loads(
            token, salt='remove-clinic', max_age=expiration
            )
        try:
            bridge = db.session.query(PersonToClinic).\
                filter_by(
                    clinic_id=clinic_id,
            ...
            if bridge:
                ...
                if clinic:
                    if bridge.is_clinic_owner is True:
                        try:
                            visits = db.session.query(Visit).\
                                filter_by(
                                    clinic_id=clinic.clinic_id).all()
                            for visit in visits:
                            ...
                            logging.warning(e)
                        try:
                            all_bridges = db.session.query(PersonToClinic).\
                                filter_by(clinic_id=clinic_id).all()
                        ...
                        return redirect(url_for('general_logic.vet_logic',
                                                choice=5, action=1))
                    elif bridge.is_clinic_owner is False:
                        db.session.delete(bridge)
                        db.session.commit()
            return redirect(url_for('general_logic.vet_logic',
                                    choice=5, action=1))
