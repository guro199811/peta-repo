"""
Auth.py:
The main purpose of this module is to authenticate users,
register new users, and manage user sessions.
...
from . import db
from .models import Person, PhonePrefixes, PersonToClinic, Clinic, Visit
...
def login():
    ...
    if request.method == "POST":
        ...
        user = Person.get_user_by_mail(mail)
        if user:
            # Check if the account is currently blocked
            if (
                user.temporary_block
                and datetime.utcnow() < user.temporary_block
            ):
                flash(
                    _(
                        "თქვენი აქაუნთი დროებით დაბლოკილია,"
            ...
            if check_password_hash(user.password, password):
                ...
                if user.login_attempts >= 10:
                    ...
                    user.temporary_block = datetime.utcnow() + timedelta(
                        minutes=30
                    )
                    db.session.commit()
                    flash(
                        _(
                            "10 წარუმატებელი ცდის შედეგად,"
                            + " აქაუნთი დაბლოკილია 30 წუთით."
                        ),
                        category="error",
                    )
                else:
                    ...
                    flash(
                        error_message.format(tries=10 - session["tries"]),
                        "error",
                    )
            ...
            flash(
                _(
                    "ელ.ფოსტა არ არის დარეგისტრირებული,"
                    + " გთხოვთ სცადოთ ხელახლა."
                ),
                "error",
            )
...
def register():
    prefixes = PhonePrefixes.get_all_prefixes()
    all_prefixes = []
    ...
    if request.method == "POST":
        ...
        if user:
            flash(
                _("მოცემული ელ.ფოსტა უკვე დარეგისტრირებულია."),
                category="error",
            )
        elif len(mail) < 4:
            flash(
                _("ემაილი უნდა შეიცავდეს მინიმუმ 4 სიმბოლოს."),
                category="error",
            )
        elif len(name) < 2:
            flash(
                _("სახელი უნდა იყოს მინიმუმ 1 ასოზე დიდი."), category="error"
            )
        elif password1 != password2:
            flash(_("გამეორებული პაროლი არაა სწორი"), category="error")
        elif len(password1) < 6:
            flash(
                _("პაროლის მინიმალური ზომა არის 6 სიმბოლო."), category="error"
            )
        elif not re.search("[a-zA-Z]", password1):
            flash(
                _("თქვენი მაროლი უნდა შეიცავდეს მინიმუმ 1 ასოს"),
                category="error",
            )
        elif not re.search("[0-9]", password1):
            flash(
                _("თქვენი პაროლი უნდა შეიცავდეს მინიმუმ 1 ციფრს"),
                category="error",
            )
        else:
            # Forming Phone Data
    ...
    return render_template(
        "sign-up.html", user=current_user, prefixes=all_prefixes
    )
...
def forgot_password():
    if request.method == "POST":
        ...
        user = Person.get_user_by_mail(email)
...
def send_password_reset_email(user):
    ...
    message = Message(
        _("პაროლის შეცვლა (Petaworld.com)"),
        sender="noreply@peta.ge",
...
def reset_password(token):
    if request.method == "POST":
        ...
        if new_password != confirm_password:
            ...
            flash(
                _("პაროლი უნდა შეიცავდეს მინიმუმ 6 სიმბოლოს"),
                category="error",
            )
    ...
    return render_template(
        "auths/reset_password.html", token=token, user=current_user
    )
...
def clinic_removal_email(clinic_id):
    ...
    try:
        person = (
            ...
            .filter_by(clinic_id=clinic_id, person_id=current_user.id)
            .one_or_none()
        )
        clinic = Clinic.get_clinic(clinic_id)
        if person.is_clinic_owner:
            message = Message(
                _("დასტური კლინიკის წაშლაზე (Petaworld.com)"),
                sender="noreply@peta.ge",
                recipients=[user.mail],
            )
            ...
            message = Message(
                _(
                    "დასტური კლინიკასთან კავშირის გაწყვეტაზე \
                                (Petaworld.com)"
                ),
                sender="noreply@peta.ge",
                recipients=[user.mail],
                    ...
                    if bridge.is_clinic_owner is True:
                        try:
                            visits = (
                                ...
                                .filter_by(clinic_id=clinic.clinic_id)
                                .all()
                            )
                            for visit in visits:
                        ...
                        return redirect(
                            url_for(
                                "general_logic.vet_logic", choice=5, action=1
                            )
                        )
