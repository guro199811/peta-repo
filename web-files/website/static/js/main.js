...
function enableEdit(petId) {
  ...
  const vaccinationInput = document.getElementById(
    `recent_vaccination_${petId}`
  );
...
function initializeMap() {
  ...
  try {
    ...
    clinicsData.forEach(function (clinic) {
      ...
      var popupContent =
        "<strong>" + clinic.clinic_name + "</strong><br>" + clinic.description;
  ...
  function onLocationFound(e) {
    ...
    L.marker(e.latlng).addTo(map).bindPopup("ჩემი ლოკაცია").openPopup();
  ...
  var findMyLocationButton = L.easyButton(
    "fa-map-marker",
    function () {
      map.locate({ setView: true, maxZoom: 16 });
    },
...
function initializeMiniMap(mapId, latitude, longitude) {
  ...
  var marker = L.marker([latitude, longitude], { icon: vetClinicIcon }).addTo(
    map
  );
}
...
function enableEditUser(user_id) {
  const inputs =
    document.querySelectorAll(`#name_${user_id}, #lastname_${user_id},
