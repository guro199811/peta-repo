    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        ...
        if search_input.isdigit():
            owner = (
                ...
                .filter_by(owner_id=int(search_input))
                .one_or_none()
            )
            person = (
                db.session.query(Person).
                filter_by(id=owner.person_id).one_or_none()
            )
        else:
            person = db.session.query(Person).\
                filter_by(mail=search_input).one_or_none()

        if person:
            owner = db.session.query(Owner).\
                filter_by(person_id=person.id).one_or_none()
            if owner:
                pets = db.session.query(Pet).\
                    filter_by(owner_id=owner.owner_id).all()
                pets_mixture = []
                for pet in pets:
            ...
            owner_html = render_template(
                "ajax_requests/owner_search_result.html",
                owner=owner,
                person=person,
                pets=pets_mixture,
            )
            ...
            return jsonify(
                html="<p>No owner found with the provided ID or Email.</p>"
                )
